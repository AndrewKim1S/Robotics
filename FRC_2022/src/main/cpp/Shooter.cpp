...
Shooter::Periodic(bool autonomous, double navX){
    ...
    // aim at the target 
    double x_off = limelightXOff+4.3+thetaOff;
...
Shooter::Aim(){
    ...
    // Set Turret movement
    if(m_autonomous){
        // std::cout << "shouldn't ever be here" << std::endl;
        double x_off = limelightXOff+4.3;
        double output = -m_turretController.Calculate(x_off);
        output = output > 0  && output > 0.38 ? 0.38: output;
        output = output < 0 && output < -0.38 ? -0.38: output;
...
Shooter::Aimed(){
    ...
    bool turretReady = abs(limelightXOff+4.3+thetaOff) < 2.5;
...
Shooter::MovementOffsetCalculation(double angle){
    ...
    // doesn't cause issues
    double offMagnitude = 0.66*sqrt((m_goalOffset.first * m_goalOffset.first) + 
        (m_goalOffset.second * m_goalOffset.second));

    // angle doesn't cause issues

    double a = 0 - angle - turnTheta;
    // sus
    double sus = atan2(m_goalOffset.second, m_goalOffset.first);
    frc::SmartDashboard::PutNumber("sus", sus);
    ...
    // frc::SmartDashboard::PutNumber("dOffCon", dOffConverted);
