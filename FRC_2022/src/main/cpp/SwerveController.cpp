...
SwerveController::calculateForward(double goal){
    return std::clamp(m_forwardcontroller.Calculate(m_forward, goal), -0.7, 0.7);
}
...
SwerveController::calculateStrafe(double goal){
    return std::clamp(-m_strafecontroller.Calculate(m_strafe, goal), -0.7, 0.7);
}
...
SwerveController::calculateRotation(double goal){
    return std::clamp(m_rotationcontroller.Calculate(m_rotation, goal), -0.6, 0.6);
}
