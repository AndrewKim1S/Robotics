...
Shooter::Periodic(bool autonomous, double navX){
    ...
    double x_off = m_limelight->getXOff()+4.3+thetaOff;
...
Shooter::Aimed(){
    ...
    bool turretReady = abs(m_limelight->getXOff()+4.3+thetaOff) < 2.5;
...
void
Shooter::Calibrate(){
    // m_speed_scale_factor = frc::SmartDashboard::GetNumber("speed", 0.0);
    // m_angle_scale_factor = frc::SmartDashboard::GetNumber("angle", 0.0);
    // frc::SmartDashboard::PutNumber("speed", m_speed_scale_factor);
    // frc::SmartDashboard::PutNumber("angle", m_angle_scale_factor);
...
Shooter::MovementOffsetCalculation(double angle){
    ...
    //make sure this is correct and in meters
    double distance = m_limelight->calculateDistance();
    // frc::SmartDashboard::PutNumber("limelightdistance", distance);
    double offMagnitude = sqrt((m_goalOffset.first * m_goalOffset.first) + 
        (m_goalOffset.second * m_goalOffset.second));

    // Time would be the third element in dataMap
    // increase m_time if shots are too short; decrease if shots are overshooting  

    // m_time * offMagnitude

    // Check this
    double a = 0 - angle - turnTheta;
    double theta1 = (3.1415/2 - atan2(m_goalOffset.second,m_goalOffset.first)) + a*3.1415/180;
    ...
    frc::SmartDashboard::PutNumber("Converted point", dOffConverted);    

    // The angle to offset the turret by
    thetaOff = asin(offMagnitude * sin(theta1) / dOff)*180/3.1415;
    turnTheta = thetaOff;

    frc::SmartDashboard::PutNumber("new distance", dOff);
    frc::SmartDashboard::PutNumber("angle offset", thetaOff);
}
...
Shooter::getFieldAng(double navX){
    const double robotAngRatio = 63000.0/360.0;
    double navX_deg = -navX;
    double turretAng = m_turret.GetSelectedSensorPosition()/robotAngRatio;
    double ang = turretAng+navX_deg+360.0;
    // frc::SmartDashboard::PutNumber("turrAng", turretAng);
    // frc::SmartDashboard::PutNumber("fieldOrientturret",ang<0?ang+360:(ang>360?ang-360:ang));
