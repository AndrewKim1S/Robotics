...
Shoot::Shoot(){
    ...
    dataMap[-10.0] = {0.53, 0.94};
    dataMap[-9.0] = {0.56, 0.94};
    dataMap[-8.0] = {0.56, 0.94};
    dataMap[-7.0] = {0.55, 0.94};
    dataMap[-6.0] = {0.54, 0.90};
    dataMap[-5.0] = {0.54, 0.86};
    dataMap[-4.0] = {0.53, 0.85};
    dataMap[-3.0] = {0.53, 0.84};
    dataMap[-2.0] = {0.52, 0.83};
    dataMap[-1.0] = {0.52, 0.83};
    dataMap[0.0] = {0.51, 0.82};
    dataMap[1.0] = {0.51, 0.82}; 
    dataMap[2.0] = {0.50, 0.78};
    dataMap[3.0] = {0.50, 0.76};
    dataMap[4.0] = {0.49, 0.75};
    dataMap[5.0] = {0.48, 0.75};
    dataMap[6.0] = {0.46, 0.75};
    dataMap[7.0] = {0.45, 0.75};
    dataMap[8.0] = {0.44, 0.73};
    dataMap[9.0] = {0.43, 0.73};
    dataMap[10.0] = {0.43, 0.73};
    dataMap[11.0] = {0.42, 0.73};
    dataMap[12.0] = {0.41, 0.70};
    dataMap[13.0] = {0.40, 0.70};
    dataMap[14.0] = {0.38, 0.70};
    dataMap[20.0] = {0.30, 0.70};
}

/*Shoot::~Shoot(){
    delete turret_limit_switch;
    delete shoot_master;
    delete shoot_slave;
    delete turret;
    delete limelight;
...
void Shoot::Aim(double robot_yaw) {
    ...
    if (x_off > 500 || abs(x_off) < 0.1 || !limelight->target_valid()){
        power = 0;
        turret->Set(ControlMode::PercentOutput, power);
    } else {
        turret->Set(ControlMode::PercentOutput, power);
    }
    frc::SmartDashboard::PutNumber("x_off", x_off);
    frc::SmartDashboard::PutNumber("tur_out_power", power);
    // ----------------------- Set the hood, default flywheel, and aim -----------------------
    servo_left.Set(angle);
    servo_right.Set(angle);
    // if(state == Shoot::State::Aiming){
    //     shoot_master->Set(ControlMode::PercentOutput, 0.80);
    //     shoot_slave->Set(ControlMode::PercentOutput, -0.80);
    // } else {
...
void Shoot::Auto(){
    while(turret->GetSelectedSensorPosition() > -6500){
...
void Shoot::Shooter_Calibrate(){
    ...
    frc::SmartDashboard::PutNumber("limelight y", limelight->getYOff());
}

//Testing function for calibrating turret values
void Shoot::Turret_Calibrate(){
    frc::SmartDashboard::PutNumber("limelight x", limelight->getXOff());
